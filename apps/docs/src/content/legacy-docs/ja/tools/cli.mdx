...
Linux インストール: ```sudo apt-get install bash-completion```
macOS インストール: ```brew install bash-completion```
</Aside>
...
| `--multi-workspace` |  | 複数のワークスペース/リポジトリをターゲットにする |
...
すべてのターゲットとワークスペースに追加されるサーバー環境変数を管理します
...
| `--retention` | `-r` | 一度に保存される結果ビルドの最大数 |
| `--run` |  | 追加後にプリビルドを一度実行する |
| `--trigger-files` | `-t` | 変更時にプリビルドを明示的にトリガーするファイルのフルパス |
| `--help` |  | daytonaのヘルプ |
...
| Long | Short | Description |
| :--- | :---- | :---------- |
| `--yes` | `-y` | プロンプトなしでDaytona Runnerパージを実行します |
| `--help` |  | daytonaのヘルプ |
...
| `--devcontainer-path` |  | フラグ値として渡されたパスでdevcontainerビルダーを自動的に割り当てる |
| `--env` |  | 環境変数を指定する（例：--env 'KEY1=VALUE1' --env 'KEY2=VALUE2' ...） |
| `--git-provider-config` |  | GitプロバイダーのコンフィギュレーションIDまたはエイリアスを指定する |
| `--label` |  | ラベルを指定する（例：--label 'label.key1=VALUE1' --label 'label.key2=VALUE2' ...） |
| `--manual` |  | Gitリポジトリを手動で入力する |
