...
- `labels` _Dict[str, str]_ - Sandboxに付けられたカスタムラベル。
...
- `memory` _int_ - Sandboxに割り当てられたメモリ量（GiB）。
- `disk` _int_ - Sandboxに割り当てられたディスク容量（GiB）。
...
- `auto_stop_interval` _int_ - 自動停止間隔（分）。
- `auto_archive_interval` _int_ - 自動アーカイブ間隔（分）。
